...
begin 
	Insert into utilizator(`cnp`,`nume`,`prenume`,`adresa`,`telefon`,`mail`,`parola`,`cont_iban`,`nr_contract`,`functie`)
		values (cnp1,nume,prenume,adresa,telefon,mail,parola,cont_iban,nr_contract,functie);
	select @id:=id from utilizator where cnp = cnp1;
	if(functie = 0)
...
drop procedure if exists sterge_activitate_grup;
DELIMITER //
